...
export default function ChessGame({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => { if (document.visibilityState === 'visible') fetchLatestState(); };
  ...
  const renderCustomBoard = () => {
    ...
    return (
      <div className="relative w-full max-w-[360px] aspect-square shadow-2xl rounded-lg overflow-hidden border-4 border-[#FFB397] mx-auto">
        <div className="absolute inset-0 bg-cover bg-center opacity-40 mix-blend-multiply bg-[#FFF8F5]" style={{ backgroundImage: "url('/AyangkuManis.png')", filter: "blur(2px)" }} />
        <div className="absolute inset-0 grid grid-cols-8 grid-rows-8 bg-white/20 backdrop-blur-sm">
          {ranks.map((rank, rowIndex) => files.map((file, colIndex) => {
              ...
              // WARNA PEACH BARU #FFB397 UNTUK KOTAK GELAP
              let squareBg = isDark ? 'bg-[#FFB397]/90' : 'bg-[#FFF8F5]/70';
              if (isSelected) squareBg = 'bg-[#FFA07A]/80 ring-inset ring-4 ring-[#FF8C66] shadow-inner';
              ...
              return (
                <div key={squareId} onClick={() => handleSquareClick(squareId)} className={`flex items-center justify-center relative cursor-pointer transition-colors duration-200 ${squareBg}`}>
                      ...
                      <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} className="w-full h-full flex items-center justify-center relative z-10 p-1">
                        {(piece.type === 'q' || piece.type === 'k') ? (
                          <div className="relative w-[85%] h-[85%] flex items-center justify-center">
                            ...
                            {/* BORDER WARNA PEACH UNTUK BIDAK */}
                            <div className={`w-full h-full rounded-full bg-cover bg-center border-2 shadow-lg ${piece.color === 'w' ? 'border-white' : 'border-[#FFB397] filter brightness-90 saturate-150'}`} style={{ backgroundImage: `url(${PIECE_IMAGES[`${piece.color}${piece.type}`]})` }} />
                          </div>
                        ) : (
                          <img 
                  ...
                  {isSelected && <Heart className="absolute text-[#FF8C66]/50 w-full h-full p-2" />}
                </div>
              );
            })
          )}
        ...
        <div className="absolute top-2 right-2 flex gap-2 z-20">
           ...
           {isBotThinking && (
               <div className="flex items-center gap-1 bg-white/80 px-2 py-0.5 rounded-full shadow-sm border border-[#FFB397]">
                   <Loader2 size={12} className="animate-spin text-[#FFB397]"/> 
                   <span className="text-[10px] font-bold text-[#FFB397]">Mikir..</span>
               </div>
           )}
        </div>
        ...
        {gameMode === 'multiplayer' && (
             <button 
                onClick={() => updateGameLocal("", false)} 
                className="absolute top-2 left-2 z-20 bg-white/50 p-1.5 rounded-full text-[#FFB397] hover:bg-white active:scale-95 transition"
             >
                <RefreshCw size={14} />
             </button>
        )}
        ...
        <AnimatePresence>
          {isGameOver && (
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
              <div className="bg-white p-6 rounded-2xl shadow-2xl text-center border-4 border-[#FFB397] w-full">
                <h3 className="text-xl font-black text-gray-800 mb-2">{gameResultMsg}</h3>
                <button onClick={resetGame} className="bg-[#FFB397] hover:bg-[#FF9E7A] text-white w-full py-3 rounded-xl font-bold flex justify-center items-center gap-2"><RotateCcw size={18} /> Main Lagi</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
  ...
  return (
    <div className="h-full flex flex-col items-center justify-start p-2 md:p-6 bg-[#FFF0E6]/50 backdrop-blur-md rounded-2xl w-full overflow-y-auto z-50 no-scrollbar">
      <button onClick={onBack} className="absolute top-4 left-4 p-2 bg-white rounded-full shadow text-[#FFB397] z-50"><ArrowLeft size={20} /></button>
      <AnimatePresence>
        {errorMsg && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="fixed top-16 left-1/2 transform -translate-x-1/2 z-[999] bg-[#FFB397] text-white px-6 py-3 rounded-xl shadow-2xl flex items-center gap-2 font-bold text-sm border-2 border-white min-w-max">
            <AlertTriangle size={18} /> {errorMsg}
          </motion.div>
        )}
      </AnimatePresence>

      <h2 className="text-2xl font-bold text-[#FFB397] mt-12 mb-6 text-center flex items-center gap-2 tracking-wide uppercase italic">
        <Heart className="text-[#FF8C66]" fill="currentColor"/> Catur Peach
      </h2>

      {status === "menu" ? (
        <div className="w-full max-w-sm flex flex-col gap-6 pb-10">
          <div className="bg-white p-5 rounded-3xl shadow-lg border border-[#FFB397]/30 flex flex-col gap-4">
            <div className="flex items-center gap-2 border-b border-[#FFB397]/20 pb-2"><Bot className="text-[#FFB397]" size={24} /><h3 className="font-bold text-gray-700">Lawan Komputer</h3></div>
            <div className="flex gap-2">
              {["cupu", "standar", "jago"].map((lvl) => (
                <button key={lvl} onClick={() => setBotDifficulty(lvl as any)} className={`flex-1 py-2 text-[10px] font-black rounded-xl capitalize border-2 transition-all ${botDifficulty === lvl ? 'bg-[#FFB397] border-[#FFB397] text-white' : 'bg-white border-[#FFB397]/30 text-[#FFB397]'}`}>{lvl.toUpperCase()}</button>
              ))}
            </div>
            <div className="flex gap-2 bg-[#FFF8F5] p-1.5 rounded-xl border border-[#FFB397]/30">
              <button onClick={() => setBotPlayerColor("white")} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${botPlayerColor === 'white' ? 'bg-white text-[#FFB397] shadow' : 'text-[#FFB397]/70 hover:text-[#FFB397]'}`}>Tim Ayang ⚪</button>
              <button onClick={() => setBotPlayerColor("black")} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${botPlayerColor === 'black' ? 'bg-[#FFB397] text-white shadow' : 'text-[#FFB397]/70 hover:text-[#FFB397]'}`}>Tim Kamu 🍑</button>
            </div>
            <button onClick={startBotMatch} className="w-full bg-gradient-to-r from-[#FFB397] to-[#FF9E7A] text-white py-3 rounded-xl font-bold flex justify-center items-center gap-2 shadow-lg active:scale-95 transition mt-1"><Play size={18} fill="currentColor"/> Main Sekarang!</button>
          </div>

          <div className="bg-white p-5 rounded-3xl shadow-lg border border-[#FFB397]/30 space-y-4">
            <div className="flex items-center gap-2 border-b border-[#FFB397]/20 pb-2"><Users className="text-[#FFB397]" size={24} /><h3 className="font-bold text-gray-700">Mabar Online</h3></div>
            <button onClick={createRoom} className="bg-[#FFF8F5] hover:bg-[#FFE5D9] text-[#FFB397] p-3 rounded-xl flex items-center justify-center gap-2 font-bold transition border border-[#FFB397]/30 text-xs">BUAT ROOM BARU</button>
            <div className="flex gap-2 mt-1">
              <input type="text" placeholder="KODE" className="flex-1 p-3 bg-[#FFF8F5] rounded-xl border border-[#FFB397]/30 text-center font-mono uppercase font-bold text-sm outline-none focus:border-[#FFB397] focus:ring-2 focus:ring-[#FFB397]/20" value={inputCode} onChange={(e) => setInputCode(e.target.value.toUpperCase())}/>
              <button onClick={joinRoom} className="bg-[#FFB397] hover:bg-[#FF9E7A] text-white px-5 rounded-xl font-bold text-sm shadow-md active:scale-95 transition">MASUK</button>
            </div>
          </div>
        </div>
      ) : (
        <div className="w-full max-w-[400px] flex flex-col items-center gap-4 pb-20">
          <div className="w-full bg-white p-3 rounded-2xl shadow-md border border-[#FFB397]/30 flex items-center justify-between px-5">
             <div className="flex items-center gap-2">
               {gameMode === "multiplayer" ? <Users size={16} className="text-[#FFB397]"/> : <Bot size={16} className="text-[#FFB397]" />}
             ...
             {gameMode === "multiplayer" && (
                <button onClick={copyCode} className="text-[#FFB397] hover:text-[#FF8C66]">
          ...
          <div className="flex gap-3 w-full px-2">
            <div className="flex-1 bg-white p-3 rounded-2xl shadow-sm border border-[#FFB397]/30 text-center flex flex-col justify-center">
              <span className="block text-[9px] text-gray-400 font-bold uppercase mb-1">Tim Kamu</span>
              <span className="font-bold text-xs text-[#FFB397]">{playerColor === 'white' ? 'Putih (Ayang)' : 'Peach (Kamu)'}</span>
            </div>
            <div className={`flex-1 p-3 rounded-2xl shadow-sm border text-center flex flex-col justify-center transition-colors ${game.turn() === (playerColor === 'white' ? 'w' : 'b') ? 'bg-[#FFF0E6] border-[#FFB397]' : 'bg-white border-[#FFB397]/30'}`}>
